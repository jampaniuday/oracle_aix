...
unless node[:oracle][:client][:latest_patch][:is_installed]
  ...
  when "redhat", "centos", "fedora"
    bash 'fetch_latest_patch_media_11R23linux' do
      user "oracli"
      group 'oinstall'
  ...
  when "aix"
    bash 'fetch_latest_patch_media_11R23aix' do
      user "oracli"
      group 'oinstall'
      cwd node[:oracle][:client][:install_dir]
      code <<-EOH
      curl -O #{node[:oracle][:client][:latest_patch][:url]}
      curl -O #{node[:oracle][:client][:opatch_update_url]}
      unzip #{File.basename(node[:oracle][:client][:latest_patch][:url])}
      EOH
    end
  end
  ...
  if !node[:oracle][:client][:response_file_url].empty?
    execute "fetch_response_file" do
      ...
      when "aix"
        command "curl -O #{node[:oracle][:client][:response_file_url]}"
      end
      user "oracli"
      group 'oracli'
      cwd node[:oracle][:client][:install_dir]
