...
node[:oracle][:rdbms][:dbs].each_key do |db|
  ...
  if node[:oracle][:rdbms][:dbbin_version] == "12c"
    ...
    if node[:oracle][:rdbms][:dbconsole][:emconfig]
      ...
      execute "set_sid_to_em-rsp_#{db}" do
        ...
        when "aix"
          command "perl -i -p -e 'if (/ORACLE_SID=$/) { s/$/#{db}/}' #{node[:oracle][:rdbms][:ora_home]}/em.rsp"
        end
        user 'oracle'
        group 'oinstall'
      end
    ...
    execute "shred_em_rsp_#{db}" do
      ...
      when "aix"
        command "/usr/linux/bin/shred -zu #{node[:oracle][:rdbms][:ora_home]}/em.rsp"
      end
      user 'root'
  ...
  execute "edit_oratabs_#{db}_config" do
    ...
    when "aix"
      command "/usr/bin/perl -i.old -p -e 'if (/^#{db}/){s/N$/Y/}' /etc/oratab"
    end
    cwd '/etc'
  end
  ...
  execute "set_oracle_sid_to_oracle_profile_#{db}" do
    ...
    when "aix"
      command "perl -i -p -e 'if ( /ORACLE_SID=$/ ) { s/$/#{db}/}' /home/oracle/.profile"
    end
    user 'oracle'
    group 'oinstall'
    environment (node[:oracle][:rdbms][:env])
  ...
  execute "set_oracle_unqname_to_oracle_profile_#{db}" do
    ...
    when "aix"
      command "perl -i -p -e 'if ( /ORACLE_UNQNAME=$/ ) { s/$/#{db}/}' /home/oracle/.profile"
    end
    user 'oracle'
    group 'oinstall'
    environment (node[:oracle][:rdbms][:env])
